...
class Helper {
  /**
     ...
     */
  static async postNull (remp, path, options) {
  ...
  /**
     ...
     * @returns {Object} the response data
     */
  static async getNull (remp, path, options) {
    const response = await remp.get(path, options)
    if (remp.isSuccess(response)) {
      return response
    } else {
      return null
  ...
  /**
     ...
     */
  static async postBoolean (remp, path, options) {
    const response = await remp.post(path, options)
    if (remp.isSuccess(response)) {
      return true
    } else {
